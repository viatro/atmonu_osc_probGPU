...
__host__
void setmix_sin(double s12,double s23,double s13,double dcp, double Mix[3][3][2]) {
...
double dmMatMat[][3], double dmMatVac[][3]) {
    ...
    */
    ...
    beta = dmVacVac[0][1]*dmVacVac[0][2] + fac*(
        dmVacVac[0][1]*(1.0 - Mix[elec][1][re]*Mix[elec][1][re] - Mix[elec][1][im]*Mix[elec][1][im]) +
        dmVacVac[0][2]*(1.0 - Mix[elec][2][re]*Mix[elec][2][re] - Mix[elec][2][im]*Mix[elec][2][im]));
    betaV = dmVacVac[0][1]*dmVacVac[0][2];
#else
    beta = dmVacVac[0][1]*dmVacVac[0][2] + fac*(
        dmVacVac[0][1]*(1.0 - Mix[elec][1][re]*Mix[elec][1][re]) +
...
extern "C" __host__
double getMixVal(int x, int y, int z) {
    return mix[x][y][z];
}
...
__host__
void init_mixing_matrix(double dm21f,double dm32f,double s12f,double s23f,double s31f,double cdf) {
...
// main kernel
__global__
void get_vacuum_probability(double mix_device[][3][2], int nutype, int beta, double *energy, int n, double path, double *osc_weight, double tdm21, double tdm32) {
    double lovere ;
...
extern "C" __host__
double* GetVacuumProb( int Alpha, int Beta , double *energy_host, int n, double Path ) {
...
extern "C" __host__
void setMNS(double x12, double x13, double x23, double m21, double m23, double Delta,/* double Energy_ ,*/ bool kSquared) {
...
__device__
void multiply_complex_matvec(double A[][3][2], double V[][2],double W[][2]) {
...
__device__
void convert_from_mass_eigenstate( int state, int flavor, double pure[][2], double mix[3][3][2] ) {
    int    i,j,k;
    double mass    [3][2];
    double conj    [3][3][2];
    int    lstate  = state - 1;
...
__device__
void get_transition_matrix(int nutypei,double Enuf,double rhof,double Lenf,double Aout[][3][2],double phase_offsetf, double mix[3][3][2], double dm[3][3]) {
...
// the colonel! (kernel...)
__global__
void propagateLinear(int Alpha, int Beta, double Path, double Density, double ye, double Mix[3][3][2], double dm[3][3], double *Energy, double *osc_w, int n) {
    // here we go
...
extern "C" __host__
void GetProb(int Alpha, int Beta, double Path, double Density, double ye, double *Energy, int n, double *oscw) {
    // copy DM matrix
